...
export function ModernTemplate({ content, accentColor, fontStyle, compact, layout = {}, sectionOrder = DEFAULT_SECTION_ORDER, columnMap }: TemplateProps) {
  ...
  // Page-level vertical padding ensures main content has top/bottom margin on
  // every page (including page 2+). Sidebar background extends ABOVE the padded
  // area using negative top positioning and an explicit A4 height (842pt) so it
  // reliably covers the full physical page on all pages without relying on
  // react-pdf's "bottom:0" which is inconsistent in multi-page layouts.
  ...
  return (
    <Document>
      <Page size="A4" style={{ flexDirection: "row", fontFamily: ff(), fontSize: c.fs, color: "#1f2937", paddingTop: pagePadV, paddingBottom: pagePadV }}>
        {/* Sidebar background — fixed, explicit A4 height (842pt) + negative top offset
            so it covers the full physical page including the padded areas. Explicit height
            is more reliable than bottom:0 which is inconsistent in react-pdf multi-page. */}
